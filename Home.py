...
with col1:
    ...
    st.page_link("pages/1_🤖_AI_Assistant.py", label="Open AI Assistant", icon="🤖")

with col2:
    st.subheader("🌡️ Sensor Dashboard")
    st.write("Live soil moisture, humidity, and temperature readings.")
    st.page_link("pages/2_🌡️_Sensor_Dashboard.py", label="Open Sensor Dashboard", icon="🌡️")

with col3:
    st.subheader("🚨 Flood / Drought Alerts")
    st.write("Early warnings based on sensor trends and rainfall forecast.")
    st.page_link("pages/3_🚨_Flood_Drought_Alerts.py", label="Open Alerts", icon="🚨")

with col4:
    st.subheader("📷 Camera Feed")
    st.write("Live snapshot from your connected field camera.")
    st.page_link("pages/4_📷_Camera_Feed.py", label="Open Camera Feed", icon="📷")

st.divider()
st.subheader("🔬 AI Disease Detection")
st.write("Upload a leaf photo or use your camera to detect crop disease.")
st.page_link("pages/5_🔬_Disease_Detection.py", label="Open Disease Detection", icon="🔬")
